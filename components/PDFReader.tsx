...
import { ChevronLeft, ChevronRight, Highlighter, MessageCircle, BrainCircuit, Languages, X, Trophy } from 'lucide-react';
    ...
    if (this.state.hasError) {
      return (
        <div className="p-8 text-center bg-white rounded-xl border border-red-200">
          <h2 className="text-xl font-bold text-red-600 mb-2">PDF Load Error</h2>
          <p className="text-slate-600 mb-4">The PDF viewer crashed. This can happen with complex textbooks.</p>
          <button onClick={() => window.location.reload()} className="px-6 py-2 bg-indigo-600 text-white rounded-lg">Reload App</button>
        </div>
      );
    }
...
export const PDFReader: React.FC<PDFReaderProps> = ({ onExplain, onQuiz, onTranslate, file, startPage = 1 }) => {
              ...
              <div className="w-full max-w-2xl mt-8 animate-in slide-in-from-bottom-6 duration-700 fade-in px-4">
                <div className="bg-slate-900 text-white rounded-2xl p-8 shadow-2xl border border-indigo-500/30 flex flex-col items-center text-center">
                  ...
                  <p className="text-slate-400 mb-8 text-base">
                    Excellent progress! Verify your mastery of these concepts with  leenjisaa's challenge.
                  </p>
                  <button
        ...
        {selection && (
          ...
          >
            <div className="bg-slate-900 text-white rounded-2xl shadow-2xl p-1.5 flex items-center gap-1 animate-in fade-in zoom-in duration-200 border border-slate-700/50 backdrop-blur-md">
              ...
              <button
                onClick={() => { onTranslate(selection.text); clearSelection(); }}
                className="px-4 py-2 hover:bg-indigo-600 rounded-xl flex items-center gap-2 text-sm font-semibold transition-all group"
              >
                <Languages size={18} className="text-indigo-400 group-hover:text-white" />
                Translate
              </button>
              <div className="w-px h-6 bg-slate-700/50 mx-1"></div>
              <button
